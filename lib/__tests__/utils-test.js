...
describe("utils", function () {
    it("gets defaults from schema and form", function () {
        ...
        var f = _utils2.default.getDefaults(schema);
    ...
    it("should handle global defaults", function () {
        ...
        var f = _utils2.default.getDefaults(schema, {}, { formDefaults: { foo: "bar" } });
        expect(f.form).toEqual(form);
    });
    ...
    it("should ignore parts of schema in ignore list", function () {
        ...
        var defaults = _utils2.default.getDefaults(schema).form;
        expect(_utils2.default.merge(schema, ["*"], { gender: true })).toEqual([defaults[0]]);
    });
    ...
    it("merges schema with different forms", function () {
        ...
        // no form is implicitly ['*']
        var defaults = _utils2.default.getDefaults(schema).form;
        expect(_utils2.default.merge(schema)).toEqual(defaults);
        expect(_utils2.default.merge(schema, ["*"])).toEqual(defaults);
        expect(_utils2.default.merge(schema, ["*", { type: "fieldset" }])).toEqual(defaults.concat([{ type: "fieldset" }]));

        // simple form
        expect(_utils2.default.merge(schema, ["gender"])).toEqual([defaults[1]]);
        expect(_utils2.default.merge(schema, ["gender", "name"])).toEqual([defaults[1], defaults[0]]);

        // change it up
        var f = _lodash2.default.cloneDeep(defaults[0]);
        f.title = "Foobar";
        f.type = "password";
        expect(_utils2.default.merge(schema, [{ key: "name", title: "Foobar", type: "password" }])).toEqual([f]);
    });
    ...
    it("should translate readOnly in schema to readonly on the merged form defintion", function () {
        ...
        var merged = _utils2.default.merge(schema, ["gender"]);
    ...
    it("should push readOnly in schema down into objects and arrays", function () {
        ...
        var merged = _utils2.default.merge(schema, ["*"]);
    ...
    it("should push readonly in form def down into objects and arrays", function () {
        ...
        var merged = _utils2.default.merge(schema, [{ key: "sub", readonly: true }]);
    ...
    it("selectOrSet test", function () {
        var result = {};
        _utils2.default.selectOrSet(["test", "tt"], result, "HI");
        expect(result.test.tt).toBe("HI");
        expect(_utils2.default.selectOrSet("test.tt", result)).toBe("HI");
        expect(_utils2.default.selectOrSet(["test", "tt"], result)).toBe("HI");

        _utils2.default.selectOrSet("my.test.fn", result, "Value");
        expect(result.my.test.fn).toBe("Value");
        expect(_utils2.default.selectOrSet("my.test.fn", result)).toBe("Value");
        expect(_utils2.default.selectOrSet(["my", "test", "fn"], result)).toBe("Value");

        _utils2.default.selectOrSet("number.is", result, 1326);
        expect(result.number.is).toBe(1326);
        expect(_utils2.default.selectOrSet("number.is", result)).toBe(1326);
        expect(_utils2.default.selectOrSet(["number", "is"], result)).toBe(1326);

        _utils2.default.selectOrSet("number.is", result, 1272);
        expect(result.number.is).toBe(1272);
        expect(_utils2.default.selectOrSet("number.is", result)).toBe(1272);
        expect(_utils2.default.selectOrSet(["number", "is"], result)).toBe(1272);
    });
    ...
    it("removeEmpty test", function () {
        ...
        _utils2.default.removeEmpty(result);
