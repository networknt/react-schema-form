...
var Array = React.createClass({
    ...
    appendToArray: function appendToArray() {
        ...
        utils.traverseForm(copy, (function (part) {
            ...
            if (part.key) {
                ...
                if (part.schema && part.schema['default']) {
    ...
    deleteFromArray: function deleteFromArray(index) {
        list.splice(index, 1);
    ...
    render: function render() {
        var arrayClasses = classNames('schema-form-array', this.props.form.htmlClass);
        var listClasses = classNames('list-group-item', this.props.form.fieldHtmlClass);
        //console.log('array classes', arrayClasses);
        //console.log('array = ', this.props.form.items);
        //console.log('render list = ', list);
        return React.createElement("div", { className: { arrayClasses: arrayClasses } }, React.createElement("h3", null, this.props.form.title), list.map((function (listItem, listIndex) {
            //console.log('listItem = ', listItem);
            React.createElement("ol", { className: "list-group" }, React.createElement("li", { className: { listClasses: listClasses } }, listItem.map((function (item, index) {
                React.createElement("button", { type: "button", className: "close pull-right" }, React.createElement("span", { "aria-hidden": "true" }, "×"), React.createElement("span", { className: "sr-only" }, "Close"));
                return MuiSchemaForm.renderSchema(item, this.props.model, index, this.props.onChange);
            }).bind(this))));
        }).bind(this)), React.createElement("div", { className: "clearfix" }, React.createElement("button", { type: "button",
            className: "btn btn-default pull-right", onClick: this.handleAdd }, React.createElement("i", { className: "glyphicon glyphicon-plus" }), "Add")));
    }
});
